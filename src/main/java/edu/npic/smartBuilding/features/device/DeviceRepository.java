...
public interface DeviceRepository extends JpaRepository<Device, Integer> {
    ...
    long countDeviceByStatus(DeviceStatus status);

    @Query("select count(d) from Device d where d.createdAt between ?1 and ?2")
    long countDeviceByDate(LocalDateTime createdAtStart, LocalDateTime createdAtEnd);
